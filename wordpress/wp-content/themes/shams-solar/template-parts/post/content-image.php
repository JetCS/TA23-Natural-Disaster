...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
  <div class="blogger p-2 mb-4">
    <?php $shams_solar_blog_layout = get_theme_mod( 'shams_solar_blog_post_layout','Default');
      if($shams_solar_blog_layout == 'Default' || $shams_solar_blog_layout == 'Center'){ ?>
      ...
      <?php } ?>
    <?php }else if($shams_solar_blog_layout == 'Image and Content'){ ?>
      <div class="row">
        <?php if(has_post_thumbnail()) { ?>
        ...
        <div class="<?php if(has_post_thumbnail()) { ?>col-lg-7 col-md-12" <?php } else { ?>col-lg-12 col-md-12" <?php } ?>>
          ...
          <?php } ?>
            <div class="text"><p class="m-0"><?php $excerpt = get_the_excerpt(); echo esc_html( shams_solar_string_limit_words( $excerpt, esc_attr(get_theme_mod('shams_solar_excerpt_number','20')))); ?></p></div>
            <?php if( get_theme_mod('shams_solar_button_text','READ MORE') != ''){ ?>
              <div class="post-link my-3 text-end">
                <a href="<?php echo esc_url( get_permalink() ); ?>"><?php echo esc_html(get_theme_mod('shams_solar_button_text','READ MORE'));?><span class="screen-reader-text"><?php echo esc_html(get_theme_mod('shams_solar_button_text','READ MORE'));?></span></a>
              </div>
